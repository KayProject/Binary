...
/** Fetch the list of bet URLs. */
const fetchBetUrls = async (): Promise<string[]> => {
  const urls: string[] = [];
...
/** Fetch a bet record from a URL. */
const fetchBetRecord = async (url: string): Promise<BetRecord | null> => {
...
export async function listBets(): Promise<BetRecord[]> {
  const urls = await fetchBetUrls();
  const rows = await Promise.all(urls.map(fetchBetRecord));
