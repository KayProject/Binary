...
const getQuotePath = (quoteId: string) => `${PREFIX}/${quoteId}.json`;
...
export const newQuoteId = () => randomBytes(12).toString("hex/WebAPI
token
doesn't

export async function writeQuote(quote: SlaQuote): Promise<void> {
  const res = await fetch(`${BLOB_API}/${getQuotePath(quote.quoteId)}`, {
    method: "PUT",
    headers: {
      ...getAuthHeaders(),
...
export async function readQuote(quoteId: string): Promise<SlaQuote | null> {
  ...
  const res = await fetch(`${getPublicBase()}/${getQuotePath(quoteId)}?v=${Date.now()}`, {
    cache: "no-store",
  });
