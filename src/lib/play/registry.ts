...
const fetchRegistryEntry = async (marketId: string): Promise<RegistryEntry | null> => {
...
export async function lookup(marketId: string): Promise<RegistryEntry | null> {
  return fetchRegistryEntry(marketId);
}
...
export async function lookupMany(
  ...
  const rows = await Promise.all(
    unique.map(async (id) => [id, await fetchRegistryEntry(id).catch(() => null)] as const)
  );
  return new Map(rows);
}
