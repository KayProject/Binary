...
const fetchBlob = async (marketId: string): Promise<RegistryEntry | null> => {
...
export async function lookupMany(
  ...
  const rows = await Promise.all(
    unique.map(async (id) => [id, await fetchBlob(id).catch(() => null)] as const)
  );
  return new Map(rows);
}
