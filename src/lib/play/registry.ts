...
const fetchAndParse = async (marketId: string): Promise<RegistryEntry | null> => {
...
export async function lookupMany(
  ...
  const rows = await Promise.all(
    unique.map(async (id) => [id, await fetchAndParse(id).catch(() => null)] as const)
  );
  return new Map(rows);
}
